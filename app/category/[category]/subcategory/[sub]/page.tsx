...
export default function SubCategoryPage() {
  ...
  const subCategoryName = paramsSearch.get("subCategoryName");
    ...
    const fetchProducts = async () => {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/product/get-active-product?subCategoryId=${sub}&page=1&limit=50${filterString}${sortQuery}`
        );
        ...
        console.log("SubCategory Product Fetch Error:", error);
      }
    };
    fetchProducts();
  }, [sub, filterString, sortBy]);
  ...
  useEffect(() => {
    const fetchFilter = async () => {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/product/fetch-filter?subCategoryId=${sub}&categoryId=${category}`
        );
        const data = await res.json();
        if (data.success) {
